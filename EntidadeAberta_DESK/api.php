  ...
  if(isset($params["frmType"]))
  {
    ...
    switch($tipo){
      ...
      case "gravaLead":
        ...
        $comment = ($cobertura == "s") ? "[STORE] - Cliente residente em localidade com cobertura. Preenchido dados iniciais." : "[STORE] - Cliente residente em localidade sem cobertura. Manifestou interesse em deixar contato salvo.";
        //payload mount
        $prepareToSend = '{
            "fields": {
                "TITLE": "LOJA ONLINE - AUTOMAÇÃO DE VENDAS",
                "NAME": "'.$payload["NAME"].'",
                "UF_CRM_1578521856": "'.$payload["UF_CRM_1578521856"].'",
                "UF_CRM_1578510751": "'.$payload["UF_CRM_1578510751"].'",
                "UF_CRM_1578521646": "'.$payload["UF_CRM_1578521646"].'",
      ...
      case "buscaPlanos":
        ...
        if(isset($_POST["payload"]))
        {
          ...
          $buscaOperadoras = mysqli_query($conexao,"select * from operadoras");
          ...
          if($entidade_aberta){
            $filtro_entidade_aberta = ',"PROPERTY_413": {
              "value": "449"
              }';
          }
          ...
          //montagem html planos
            ...
            foreach ($resultPlanos as $plano) {
              ...
              $operadoras_entidade_aberta = ["7058","7357","7221"];
              ...
              {
                $html_planos .= '
                <div class="gallery_product col-lg-4 col-md-4 col-sm-6 col-xs-6 filter slide acomoda'.$plano["PROPERTY_161"]["value"].' copart'.$plano["PROPERTY_366"]["value"].' " data-order="'.$line.'" data-price="'.$plano["PRICE"].'" data-operadoraid="'.$operadoraID.'">
                  <table class="borda_plano">
                    <tr>
                        <td height="5"></td>
                    </tr>
                    <tr>
                        <td><img style="padding:0 5px;" src="'.$plano["imgSrc"].'"></td>
                        <td>
                            <table>
                                <tr>
                    ...
                    <tr>
                        <td class="texto2" colspan="2" ><strong>R$ '.number_format($plano["PRICE"],2,",",".").'</strong><br>
                        <input type="button" name="next" class="btn filter-button2 choosePlan" value="Escolher" data-plano=\''.json_encode($plano).'\' style="width: 80%;">  </td>
                    </tr>
                    <tr>
      ...
      case "converteLead":
        ...
        $payloadContato = '{
            "fields": {
                ...
                "UF_CRM_1578521646": "'.strtoupper($payload["UF_CRM_1578521646"]).'",
